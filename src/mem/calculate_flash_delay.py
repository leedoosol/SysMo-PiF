...
###########################################################################################################################
def calculate_flash_read_io_time(
		mem_config,
...
#	read_bw /= clock_speed
	...
	# Get flash read latency
	
	flash_read_latency = mem_config.t_slc_read
	if  flash_dense == 'SLC':
		flash_read_latency = mem_config.t_slc_read
	elif flash_dense == 'MLC':
		flash_read_latency = mem_config.t_mlc_read
	elif flash_dense == 'TLC':
		flash_read_latency = mem_config.t_tlc_read
	
	flash_read_latency_once = flash_read_latency
	flash_read_latency /= 1000000 # us -> s
	...
	# Change memory bandwidth (bytes/sec -> bytes/cycle)
	# bandwidth(bytes per sec) / ( precision(byte) * clock speed(Hz = clocks per sec) )
	read_bw = (flash_page_size/flash_read_latency)*mem_config.flash_plane_num
	total_read_once = (flash_page_size*mem_config.flash_plane_num)/precision
...
#						t_fill_sram_buffer = (len(sram)/read_bw) * time_per_cycle
						...
						read_cnt    = math.ceil(len(sram)/total_read_once)
						t_fill_sram_buffer = read_cnt * flash_read_latency_once
...
###########################################################################################################################
def calculate_flash_write_io_time(
		mem_config,
		ofmap_sram_size = 64,
		data_width_bytes = 1,
		write_bw  = 10,
...
#	write_bw /= clock_speed
	...
	# Get flash write latency
	flash_write_latency = mem_config.t_slc_write

	if  flash_dense == 'SLC':
		flash_write_latency = mem_config.t_slc_write
	elif flash_dense == 'MLC':
		flash_write_latency = mem_config.t_mlc_write
	elif flash_dense == 'TLC':
		flash_write_latency = mem_config.t_tlc_write

	flash_write_latency_once = flash_write_latency
	flash_write_latency /= 1000000 # us -> s
	...
	# Change memory bandwidth (bytes/sec -> bytes/cycle)
	# bandwidth(bytes per sec) / ( precision(byte) * clock speed(Hz = clocks per sec) )
	write_bw = (flash_page_size/flash_write_latency)*mem_config.flash_plane_num
	write_bw /= precision
	write_bw /= clock_speed

	flash_page_size /= precision
	total_write_once = flash_page_size*mem_config.flash_plane_num
...
#			print("haha")
			...
			if len(sram_buffer[draining_buf]) > 0:
				...
				write_cnt    = math.ceil(len(sram_buffer[draining_buf])/total_write_once)
				t_drain_sram_buffer = write_cnt * flash_write_latency_once
#				t_drain_sram_buffer = (len(sram_buffer[draining_buf]) / write_bw) * time_per_cycle
				...
				print("last_clk = %7d"             %(last_clk)        + "\t " + \
					  ...
					  "t_available = %10f"         %(idle_delay_time+t_available) + "\t " + \
					  "t_drain_sram_buffer = %10f" %(t_drain_sram_buffer)
				)
				'''

				idle_delay_time = 0 # consider flash read/write request crash

				t_available += idle_delay_time
#				print("t_available = %10f"%(t_available))
#				print("t_drain_sram_buffer = %10f" %(t_drain_sram_buffer))
			
				if t_drain_sram_buffer > t_available:
					temp_time     = t_drain_sram_buffer - t_available
#					print("temp_time=%10f" %(temp_time))
					t_delayed_io += temp_time
#				print("t_delayed_io = %10f" %(t_delayed_io))
				# Drain the data
