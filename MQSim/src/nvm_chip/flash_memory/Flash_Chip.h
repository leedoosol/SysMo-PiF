...
namespace NVM
{
	namespace FlashMemory
	{
		...
		public:
			...
			void EndCMDXfer(Flash_Command* command, sim_time_type suspendTime, bool is_GC_WL)//End transferring write data to the Flash chip
			{
				...
				start_command_execution(command, suspendTime, is_GC_WL);

				this->lastTransferStart = INVALID_TIME;
			}

			//doodu is_GC_WL (tunneling from 'register_sim_event() of send_command_~ to here)
			void EndCMDDataInXfer(Flash_Command* command, bool is_GC_WL)//End transferring write data of a group of multi-plane transactions to the Flash chip
			{
				this->STAT_totalXferTime += (Simulator->Time() - this->lastTransferStart);
				if (this->idleDieNo != die_no)
					STAT_totalOverlappedXferExecTime += (Simulator->Time() - lastTransferStart);
				...
				start_command_execution(command, 0, is_GC_WL);
...
#ifdef ACCEL_MODEL
					//std::cout << _readLatency[latencyType] << std::endl;
					return _readLatency[latencyType] + _RBSignalDelayRead + lat_model->cal_latency(MAT2MAT);
#else
		...
		private:
			...
			void start_command_execution(Flash_Command* command, sim_time_type suspendTime, bool is_GC_WL);
