...
FILE* pfile = fopen("command_log.txt", "a");
#define COMMAND_LOG 0
int cnt_log = 0;
bool is_allGC = 0;
double total_GC_W_parallel = 0; 
double total_USR_W_parallel = 0;
double total_GC_R_parallel = 0;
double total_USR_R_parallel = 0;
double cnt_send_command = 0;
double cnt_send_command_R = 0;
#endif
...
namespace SSD_Components {
	...
	inline ChipStatus NVM_PHY_ONFI_NVDDR2::GetChipSubStatus_W(NVM::FlashMemory::Flash_Chip* chip)
	{
		return bookKeepingTable[chip->ChannelID][chip->ChipID].subStatus_W;
	}
	...
	void NVM_PHY_ONFI_NVDDR2::Send_command_to_chip(std::list<NVM_Transaction_Flash*>& transaction_list)
	{
		...
		* then something illegarl is happening*/
		...
		int GC_WL_W = 0; int GC_WL_R = 0;
		int USR_WL_W = 0; int USR_WL_R = 0;
		
		sim_time_type suspendTime = 0;
		bool GC_WL_exists = false; //doodu flag 0211
		if (!dieBKE->Free)
		{
			if (transaction_list.front()->SuspendRequired)
			{
				switch (dieBKE->ActiveTransactions.front()->Type) //현재 처리되고 있는 transaction이 dieBKE->ActiveTransactions.front(). maybe..
				{
...
//					suspendTime = target_channel->ProgramSuspendCommandTime + targetChip->GetSuspendProgramTime();
					...
					//flag0211
					for (std::list<NVM_Transaction_Flash*>::iterator it = dieBKE->ActiveTransactions.begin(); it != dieBKE->ActiveTransactions.end(); it++) {
						if ((*it)->Source == Transaction_Source_Type::GC_WL) {
							GC_WL_exists = true;
						}
					}
					if(GC_WL_exists) Stats::IssuedSuspendProgramCMD_GC++;
					GC_WL_exists = false;
					...
					for (std::list<NVM_Transaction_Flash*>::iterator it = dieBKE->ActiveTransactions.begin(); it != dieBKE->ActiveTransactions.end(); it++) {
						...
						else if ((*it)->Source == Transaction_Source_Type::USERIO) {
							std::cout << "USER_IO, ";
						}
					}
					std::cout << " | ";
...
//					suspendTime = target_channel->EraseSuspendCommandTime + targetChip->GetSuspendEraseTime();
		...
		//std::cout <<"[DOODU] transaction_list observation"<< std::endl;
		int erase_tr_cnt = 0;
		int subblock_erase_tr_cnt = 0;
		for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
			it != transaction_list.end(); it++){
			//if ((*it)->Type == Transaction_Type::READ) std::cout << "READ,";
			//else if ((*it)->Type == Transaction_Type::WRITE) std::cout << "WRITE,";
			if ((*it)->Type == Transaction_Type::ERASE) {
				erase_tr_cnt++;
				if ((*it)->isSubblock_Erase_Tr == true) {
					subblock_erase_tr_cnt++;
					//std::cout << "Block offset of PPA: " << ((*it)->PPA)/256<<" (256 blocks)" << std::endl;
					//std::cout << (*it)->Physical_address_determined;
				}
			}
		}
		////if(erase_tr_cnt!=0) std::cout <<"[DOODU] Plane-level GC parallelism count: "<<erase_tr_cnt<<"subblock_erase_tr_cnt: "<< subblock_erase_tr_cnt << std::endl;

		/*
		if (transaction_list.size() != 1 && transaction_list.front()->Type == Transaction_Type::ERASE) {
			for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
				it != transaction_list.end(); it++) {
				//if ((*it)->Type == Transaction_Type::READ) std::cout << "READ,";
				//else if ((*it)->Type == Transaction_Type::WRITE) std::cout << "WRITE,";
				//if ((*it)->Type == Transaction_Type::ERASE && (*it)->Physical_address_determined) {
				std::cout << "Block offset of : " << (*it)->Address.BlockID<<", ";
				//std::cout << (*it)->Physical_address_determined;
			}
		std::cout<< std::endl;
		}
		*/
		...
		bool isGCW = false;
		bool isAllPlanesGCW = false; //whether all transactions to all planes (multi-plane unit) are GC write or not 
		int cnt_GCw = 0;
		...
		case Transaction_Type::READ:
#if NEW_SUBMIT
			if (transaction_list.size() > 0) { //Multiplane write (if user Write >= 2 )
				for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin(); it != transaction_list.end(); it++) {
					if ((*it)->Source == Transaction_Source_Type::GC_WL) {
						GC_WL_R++; //std::cout << "GC W,";
					}
					else if ((*it)->Source == Transaction_Source_Type::USERIO) {
						USR_WL_R++; ///std::cout << "USR W,";
					}
				}
				//std::cout << std::endl;
			}
			total_GC_R_parallel += GC_WL_R; total_USR_R_parallel += USR_WL_R; 
			cnt_send_command_R++;
			Stats::avg_GC_R_parallel = total_GC_R_parallel / cnt_send_command_R;
			Stats::avg_USR_R_parallel = total_USR_R_parallel / cnt_send_command_R;
#endif
			...
			if (chipBKE->OngoingDieCMDTransfers.size() == 0)
			{
				...
				if (isGCW) {
					...
					Simulator->Register_sim_event(Simulator->Time(), this,
						dieBKE, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED, 0, isGCW, NOT_ALL_GC_W);
					/*
					chipBKE->Last_transfer_finish_time = Simulator->Time() + suspendTime + (target_channel->ReadCommandTime[transaction_list.size()])/ DEVISION_FACTOR;
					Simulator->Register_sim_event(Simulator->Time() + suspendTime + (target_channel->ReadCommandTime[transaction_list.size()])/DEVISION_FACTOR, this,
						dieBKE, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED, 0, isGCW);
					*/
				}
				else {
			...
			else
			{
				...
				if (isGCW) {
					...
					/*
					dieBKE->DieInterleavedTime = suspendTime + (target_channel->ReadCommandTime[transaction_list.size()])/ DEVISION_FACTOR;
					chipBKE->Last_transfer_finish_time += suspendTime + (target_channel->ReadCommandTime[transaction_list.size()])/ DEVISION_FACTOR;
					*/
				}
				else if (isGCW == false) {
					...
					std::cout << "check send_command_to_chip read [doodu]" << std::endl;
					exit(1);
				}
#else
				dieBKE->DieInterleavedTime = suspendTime + target_channel->ReadCommandTime[transaction_list.size()];
...
#if REMOVE_LAT_GC_W //0216
			if (isGCW) {
				//std::cout << "shorten" << std::endl;
				dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time;
				//dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time + (targetChip->Get_command_execution_latency(dieBKE->ActiveCommand->CommandCode, dieBKE->ActiveCommand->Address[0].PageID))/ DEVISION_FACTOR; //flag0217
...
//			if ( transaction_list.front()->Address.ChannelID == 3 && transaction_list.front()->Address.ChipID == 3 )
		...
		case Transaction_Type::WRITE:
			...
			isAllPlanesGCW = false;
			if (cnt_GCw == transaction_list.size()) { 
				isAllPlanesGCW = true; /*std::cout << "isAllPlanesGCW=true";*/ 
			} 
			cnt_GCw = 0;
			...
			if (transaction_list.size() > 0) { //Multiplane write (if user Write >= 2 )
				for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin(); it != transaction_list.end(); it++) {
					if ((*it)->Source == Transaction_Source_Type::GC_WL) {
						GC_WL_W++; //std::cout << "GC W,";
					}
					else if ((*it)->Source == Transaction_Source_Type::USERIO) {
					    USR_WL_W++; ///std::cout << "USR W,";
					}
				}
				//std::cout << std::endl;
			}
			total_GC_W_parallel += GC_WL_W; total_USR_W_parallel += USR_WL_W; cnt_send_command++;
			Stats::avg_GC_W_parallel = total_GC_W_parallel/cnt_send_command;
			Stats::avg_USR_W_parallel = total_USR_W_parallel / cnt_send_command;

			if ((GC_WL_W>1) && (transaction_list.size() == GC_WL_W || ((transaction_list.size() - 1) == GC_WL_W))) {
				Stats::IssuedMultiplaneGCProgramCMD++;
			}

			else if (transaction_list.size() == 1 && transaction_list.front()->Source == Transaction_Source_Type::GC_WL) {
				//do nothing
			}
#endif
...
#if REMOVE_LAT_GC_W //0213
						//std::cout << "isGCW=1" << std::endl;
						isGCW = true;
#endif
						break;
					}
...
#if NEW_SUBMIT
				/* user, gc pgm이 transaction_list에  섞일 수 있다.
				for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin(); it != transaction_list.end(); it++) {
					if ((*it)->Source == Transaction_Source_Type::GC_WL) {
						std::cout << "G ";
					}
				...
				*/
				...
				for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
					...
					if (isGCW) {
						//do nothing
						/*
						(*it)->STAT_transfer_time += (target_channel->ProgramCommandTime[transaction_list.size()] + NVDDR2DataInTransferTime((*it)->Data_and_metadata_size_in_byte, target_channel)) / DEVISION_FACTOR; //flag0217
						data_transfer_time += (NVDDR2DataInTransferTime((*it)->Data_and_metadata_size_in_byte, target_channel)) / DEVISION_FACTOR; //flag0217
						*/
					}
					else if (isGCW == false) {				
						...
						std::cout << "check send_command write [doodu error]" << std::endl;
					}
#else
					(*it)->STAT_transfer_time += target_channel->ProgramCommandTime[transaction_list.size()] + NVDDR2DataInTransferTime((*it)->Data_and_metadata_size_in_byte, target_channel);
					data_transfer_time += NVDDR2DataInTransferTime((*it)->Data_and_metadata_size_in_byte, target_channel);
				...
				if (chipBKE->OngoingDieCMDTransfers.size() == 0)
				{
					...
					if (isGCW) {
						/*
						chipBKE->Last_transfer_finish_time = Simulator->Time() + suspendTime + (target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time)/ DEVISION_FACTOR;
						Simulator->Register_sim_event(Simulator->Time() + suspendTime + (target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time)/ DEVISION_FACTOR,
							this, dieBKE, (int)NVDDR2_SimEventType::PROGRAM_CMD_ADDR_DATA_TRANSFERRED, 0, isGCW);
						*/
						
						chipBKE->Last_transfer_finish_time = Simulator->Time();
						Simulator->Register_sim_event(Simulator->Time(),
							this, dieBKE, (int)NVDDR2_SimEventType::PROGRAM_CMD_ADDR_DATA_TRANSFERRED, 0, isGCW, isAllPlanesGCW);
						...
						std::cout << "check send_command write command [doodu error]" << std::endl;
					}
#else
					chipBKE->Last_transfer_finish_time = Simulator->Time() + suspendTime + target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time;
					Simulator->Register_sim_event(Simulator->Time() + suspendTime + target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time,
				...
				else
				{
					...
					if (isGCW) {
						/*
						dieBKE->DieInterleavedTime = suspendTime + (target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time)/ DEVISION_FACTOR;
						chipBKE->Last_transfer_finish_time += suspendTime + (target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time)/ DEVISION_FACTOR;
						*/
						/*
						do nothing
						*/
				...
				if (isGCW) {
					...
					std::cout<<"check send_command_to_chip [doodu]"<<std::endl;
					exit(1);
				}
#else
				dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time + targetChip->Get_command_execution_latency(dieBKE->ActiveCommand->CommandCode, dieBKE->ActiveCommand->Address[0].PageID);
...
//				if ( transaction_list.front()->Address.ChannelID == 3 && transaction_list.front()->Address.ChipID == 3 )
			...
			else//Copyback write for GC
			{
				...
				if (chipBKE->OngoingDieCMDTransfers.size() == 0)
				{
					...
					chipBKE->Last_transfer_finish_time = Simulator->Time() + suspendTime + target_channel->ReadCommandTime[transaction_list.size()];
					Simulator->Register_sim_event(Simulator->Time() + suspendTime + target_channel->ReadCommandTime[transaction_list.size()], this,
						dieBKE, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED, 0, isGCW, isAllPlanesGCW);
				}
				else
				{
					dieBKE->DieInterleavedTime = suspendTime + target_channel->ReadCommandTime[transaction_list.size()];
...
//				if ( transaction_list.front()->Address.ChannelID == 3 && transaction_list.front()->Address.ChipID == 3 )
			...
			break;
		case Transaction_Type::ERASE: //한번에 최대 plane갯수까지만 CMD_ERASE_BLOCK_MULTIPLANE

			for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin(); it != transaction_list.end(); it++) { //0113flag
			//	std::cout << "(send_command~) Cha_Chip_D_PL: " << (*it)->Address.ChannelID << ", " << (*it)->Address.ChipID << ", " << (*it)->Address.PlaneID << ", " << (*it)->Address.BlockID << ", "<<(*it)->isSubblock_Erase_Tr << std::endl;
			}
			...
			if (chipBKE->OngoingDieCMDTransfers.size() == 0)
			{
				...
				Simulator->Register_sim_event(Simulator->Time() + suspendTime + target_channel->EraseCommandTime[transaction_list.size()],
					this, dieBKE, (int)NVDDR2_SimEventType::ERASE_SETUP_COMPLETED, 0, NOT_GC_WL, NOT_ALL_GC_W);
...
//			if ( transaction_list.front()->Address.ChannelID == 3 && transaction_list.front()->Address.ChipID == 3 )
		...
		int cnt_gc = 0;
		for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin(); it != transaction_list.end(); it++) {
			if (transaction_list.front()->Source == Transaction_Source_Type::USERIO) cnt_user++;
			else if (transaction_list.front()->Source == Transaction_Source_Type::GC_WL) cnt_gc++;
		}

		if (transaction_list.size() == cnt_gc && transaction_list.size()!=0 ) is_allGC = true;
		else is_allGC = false;
		...
		{
			case Transaction_Type::READ:
				fprintf(pfile, "%d R %u %u %u %d %d %d\n", cnt_log, transaction_list.size(), target_channel->ReadCommandTime[transaction_list.size()], dieBKE->Expected_finish_time,cnt_user, cnt_gc, is_allGC);
				cnt_log++;
				break;
			case Transaction_Type::WRITE:
				fprintf(pfile, "%d W %u %u %u %d %d %d\n", cnt_log, transaction_list.size(), target_channel->ProgramCommandTime[transaction_list.size()], dieBKE->Expected_finish_time,cnt_user, cnt_gc, is_allGC);
				cnt_log++;
				break;
			case Transaction_Type::ERASE:
				fprintf(pfile, "%d E %u %u %u %d %d %d\n", cnt_log, transaction_list.size(), target_channel->EraseCommandTime[transaction_list.size()], dieBKE->Expected_finish_time, cnt_user, cnt_gc, is_allGC);
				cnt_log++;
				break;
			default:
				break;
	...
	void NVM_PHY_ONFI_NVDDR2::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
		...
		case NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED:
			...
			{
				targetChip->EndCMDXfer(dieBKE->ActiveCommand, targetChip->GetSuspendEraseTime(), ev->is_GCWL); 
				dieBKE->ERS_SuspendTrigger = false;
			}
			else if (true == dieBKE->PGM_SuspendTrigger)
			{
				targetChip->EndCMDXfer(dieBKE->ActiveCommand, targetChip->GetSuspendProgramTime(), ev->is_GCWL);
				dieBKE->PGM_SuspendTrigger = false;
			}
			else
			{
				targetChip->EndCMDXfer(dieBKE->ActiveCommand, 0, ev->is_GCWL);
			}

			for (auto tr : dieBKE->ActiveTransactions) {
				tr->STAT_execution_time = dieBKE->Expected_finish_time - Simulator->Time();
				////total_read_requests++;
				////std::cout << "[DOODU] total_read_requests: " << total_read_requests << std::endl;
			}
			...
			else
			{
				...
				//doodu chipBKE->subStatus = ChipStatus::WAIT_FOR_GC_READ;
...
//			if ( nCH_ID == 3 && nCHIP_ID == 3 )
		...
		case NVDDR2_SimEventType::ERASE_SETUP_COMPLETED:
			...
			targetChip->EndCMDXfer(dieBKE->ActiveCommand, 0,NOT_GC_WL);
			for (auto &tr : dieBKE->ActiveTransactions)
				tr->STAT_execution_time = dieBKE->Expected_finish_time - Simulator->Time();
			chipBKE->OngoingDieCMDTransfers.pop();
			chipBKE->No_of_active_dies++;
...
//			if ( nCH_ID == 3 && nCHIP_ID == 3 )
		...
		case NVDDR2_SimEventType::PROGRAM_COPYBACK_CMD_ADDR_TRANSFERRED:
			...
			targetChip->EndCMDDataInXfer(dieBKE->ActiveCommand, ev->is_GCWL);
			for (auto &tr : dieBKE->ActiveTransactions)
				tr->STAT_execution_time = dieBKE->Expected_finish_time - Simulator->Time();
			chipBKE->OngoingDieCMDTransfers.pop();
			chipBKE->No_of_active_dies++;
...
#if NEW_SUBMIT
				if (ev->is_allGCW) {
					chipBKE->subStatus_W = ChipStatus::ALL_PLANES_GCW;
				}
#endif
			}
...
//			if ( nCH_ID == 3 && nCHIP_ID == 3 )
		...
		if (WaitingCopybackWrites[channel_id].size() > 0)
		{
			...
			Simulator->Register_sim_event(Simulator->Time() + this->channels[channel_id]->ProgramCommandTime[waitingBKE->ActiveTransactions.size()],
				this, waitingBKE, (int)NVDDR2_SimEventType::PROGRAM_COPYBACK_CMD_ADDR_TRANSFERRED, 0, NOT_GC_WL, NOT_ALL_GC_W);
	...
	inline void NVM_PHY_ONFI_NVDDR2::handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip* chip, NVM::FlashMemory::Flash_Command* command)
	{
		...
		case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
			...
			if (_my_instance->channels[chip->ChannelID]->GetStatus() == BusChannelStatus::IDLE)
			{
				...
				Simulator->Register_sim_event(Simulator->Time() + _my_instance->channels[chip->ChannelID]->ProgramCommandTime[dieBKE->ActiveTransactions.size()],
					_my_instance, dieBKE, (int)NVDDR2_SimEventType::PROGRAM_COPYBACK_CMD_ADDR_TRANSFERRED, 0, NOT_GC_WL, NOT_ALL_GC_W);
				chipBKE->OngoingDieCMDTransfers.push(dieBKE);
		...
		case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
		{
			...
			if (chipBKE->No_of_active_dies == 0 && chipBKE->WaitingReadTXCount == 0) {
				chipBKE->Status = ChipStatus::IDLE;
#if NEW_SUBMIT
				chipBKE->subStatus_W = ChipStatus::IDLE;
#endif
			}
	...
	inline void NVM_PHY_ONFI_NVDDR2::transfer_read_data_from_chip(ChipBookKeepingEntry* chipBKE, DieBookKeepingEntry* dieBKE, NVM_Transaction_Flash* tr)
	{
		...
		Simulator->Register_sim_event(Simulator->Time() + NVDDR2DataOutTransferTime(tr->Data_and_metadata_size_in_byte, channels[tr->Address.ChannelID]),
			this, dieBKE, (int)NVDDR2_SimEventType::READ_DATA_TRANSFERRED, 0, NOT_GC_WL, NOT_ALL_GC_W);
	...
	void NVM_PHY_ONFI_NVDDR2::perform_interleaved_cmd_data_transfer(NVM::FlashMemory::Flash_Chip* chip, DieBookKeepingEntry* bookKeepingEntry)
	{
		...
		case Transaction_Type::READ:
			...
			Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime,
				this, bookKeepingEntry, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED, 0, NOT_GC_WL, NOT_ALL_GC_W);
			break;
		case Transaction_Type::WRITE:
			if (((NVM_Transaction_Flash_WR*)bookKeepingEntry->ActiveTransactions.front())->RelatedRead == NULL) {
				...
				Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime,
					this, bookKeepingEntry, (int)NVDDR2_SimEventType::PROGRAM_CMD_ADDR_DATA_TRANSFERRED, 0, NOT_GC_WL, NOT_ALL_GC_W);
			}
			else
			{
				chip->StartCMDXfer();
				...
				Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime, this,
					bookKeepingEntry, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED, 0, NOT_GC_WL, NOT_ALL_GC_W);
			}
			break;
		case Transaction_Type::ERASE:
			chip->StartCMDXfer();
			bookKeepingTable[chip->ChannelID][chip->ChipID].Status = ChipStatus::CMD_IN;
			Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime,
				this, bookKeepingEntry, (int)NVDDR2_SimEventType::ERASE_SETUP_COMPLETED, 0, NOT_GC_WL, NOT_ALL_GC_W);
			break;
		default:
