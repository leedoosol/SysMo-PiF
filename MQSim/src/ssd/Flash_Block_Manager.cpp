...
int total_subblock_erase_cnt = 0;
int primary_cnt = 0;
int sub_cnt = 0;
unsigned long total_Parallel_GC = 0;
#define SUBBLOCK 1
#endif
...
namespace SSD_Components
{
	...
	void Flash_Block_Manager::Allocate_block_and_page_in_plane_for_user_write(const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		...
		if(plane_record->Data_wf[stream_id]->Current_page_write_index == pages_no_per_block)//The current write frontier block is written to the end
		{
			...
			//int nextVictim = -1;
			struct cand_blks nextVictims; nextVictims.cnt = 0;
			nextVictims = gc_and_wl_unit->Check_gc_required(plane_record->Get_free_block_pool_size(), page_address, -1);
			//nextVictims = gc_and_wl_unit->Check_gc_required(plane_record->Get_free_block_pool_size(), page_address, -1);
#if NEW_SUBMIT
			int max_subblk_erase_cnt = MAX_SUBBLOCK_ERS_CNT;
			int count = 0;
			//std::cout << "additional victims: "<<nextVictims.cnt << std::endl;
			if(nextVictims.cnt > 0 ) {
				for (int i = 0; i < nextVictims.cnt; i++) {
					if (count < max_subblk_erase_cnt) {
						//std::cout << nextVictims.cnt << ", " << count << std::endl;
						gc_and_wl_unit->Check_gc_requiredForce(plane_record->Get_free_block_pool_size(), page_address, nextVictims.blocks[i], SUBBLOCK);
						std::cout << "Procced more than 1 subblock" << std::endl;
						count++;
						sub_cnt++;
					}
				}
			}
			else if (nextVictims.cnt < 0) {
				//PRINT_ERROR("Choose GREEDY or error happened from Check_gc_required [DOODU]");
			}
			total_Parallel_GC = (primary_cnt + sub_cnt);
			Stats::Parallelism_GC_level = (total_Parallel_GC) / primary_cnt;
			//std::cout << total_Parallel_GC << ", "<<primary_cnt << ", " << Stats::Parallelism_GC_level << std::endl;
			
			gc_and_wl_unit->Check_gc_required(plane_record->Get_free_block_pool_size(), page_address,-1);
	...
	void Flash_Block_Manager::Allocate_block_and_page_in_plane_for_gc_write(const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		...
		//gc_and_wl_unit->Check_gc_required(plane_record->Get_free_block_pool_size(), page_address, -1); //flag0221
		plane_record->Check_bookkeeping_correctness(page_address);
	}
