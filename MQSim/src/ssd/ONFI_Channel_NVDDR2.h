...
#define NVDDR2DataInTransferTime(X,Y) ((X / Y->ChannelWidth / 2) * Y->TwoUnitDataInTime_PS / 1000)
#define NVDDR2DataOutTransferTime(X,Y) ((X / Y->ChannelWidth / 2) * Y->TwoUnitDataOutTime_PS /1000)
...
namespace SSD_Components
{
	...
	public:
		ONFI_Channel_NVDDR2(flash_channel_ID_type channelID, unsigned int chipCount, NVM::FlashMemory::Flash_Chip** flashChips, unsigned int ChannelWidth,
			sim_time_type t_RC_PS = 6000, sim_time_type t_DSC_PS = 6000,
		...
		sim_time_type TwoUnitDataOutTime_PS; //The DDR delay for two-unit device data out
		...
		sim_time_type TwoUnitDataInTime_PS; //The DDR delay for two-unit device data in
	...
	private:
		...
		sim_time_type t_RC_PS; //Average RE cycle time, e.g. 6ns
		sim_time_type t_DSC_PS; //Average DQS cycle time, e.g. 6ns
