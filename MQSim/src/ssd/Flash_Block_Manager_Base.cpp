...
//#if NEW_SUBMIT
	flash_block_ID_type Flash_Block_Manager_Base::Get_2ndColdest_block_id(const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		unsigned int min_erased_block = 0;
		unsigned int min_erased_block2_v = UINT32_MAX;
		unsigned int min_erased_block2_i = 0;
		...
		for (unsigned int i = 0; i < block_no_per_plane; i++) {
			if (plane_record->Blocks[i].Erase_count < min_erased_block2_v && (i != min_erased_block)) {
				min_erased_block2_v = plane_record->Blocks[i].Erase_count;
				min_erased_block2_i = i;
			}
		}
		/*
		std::cout << min_erased_block << ", " << min_erased_block2_i << std::endl;
		for (int i = 0; i < 5; i++) {
			std::cout << plane_record->Blocks[i].Erase_count << ",";
		}
		std::cout << std::endl;
		*/
		return min_erased_block2_i;
	}
//#endif
