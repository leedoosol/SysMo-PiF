		...
		case Transaction_Source_Type::CACHE:
			...
			if (_my_instance->block_manager->Block_has_ongoing_gc_wl(transaction->Address))
			{
				if (_my_instance->block_manager->Can_execute_gc_wl(transaction->Address))
				{
					...
					//std::cout << "Total_gc_executions++ in the GC_and_WL_Unit_Base.cpp" << std::endl;
#if NEW_SUBMIT
					if(transaction->isSubblock_Erase_Tr) {
						//do nothing 
					}
					else {
...
//					assert(false);
		...
		case Transaction_Type::ERASE:
			...
			//std::cout << "Erase block" << std::endl; //flag0211
			...
			//_my_instance->run_static_wearleveling(transaction->Address);
#if SOFT_ISOLATION
			//_my_instance->make_soft_isolation(transaction->Address, transaction->Address.BlockID);
#endif 
...
//			if (pbke->Get_free_block_pool_size() <= 15)
			//std::cout << "free block size: " <<pbke->Get_free_block_pool_size() << std::endl;
#if (PGM_SUS_ENABLE || MLC_TLC_GC_THRESHOLD)
			if (pbke->Get_free_block_pool_size() <= 10) //34
#else
			if (pbke->Get_free_block_pool_size() <= 10) //15?
#endif
			{
//				std::cout << "GC Trigger : " << transaction->Address.ChannelID << " : " << transaction->Address.ChipID << " : " << transaction->Address.PlaneID << " : " << pbke->Get_free_block_pool_size() << std::endl;
				std::cout << "check_gc_required 2" << std::endl;
				_my_instance->Check_gc_required(pbke->Get_free_block_pool_size(), transaction->Address,-1);
			}
			break;
		}
	}
...
//			std::cout << "Min FREE BLK : " << plane_address.ChannelID << " : " << plane_address.ChipID << " : " << plane_address.PlaneID << " : " << block_manager->Get_pool_size(plane_address) << std::endl;
...
#if SOFT_ISOLATION
	void GC_and_WL_Unit_Base::make_soft_isolation(const NVM::FlashMemory::Physical_Page_Address plane_address, flash_block_ID_type blockID)
	{
		...
		//wl_candidate_block_id = blockID/2;
		if (blockID == 0) wl_candidate_block_id = 1;
		else if (blockID == (block_no_per_plane - 1)) wl_candidate_block_id = blockID - 1;
		else {
			wl_candidate_block_id = blockID - 1;
		}
		if (wl_candidate_block_id < 0) { 
			std::cout << "[DOODU_DEBUG] error in 'make_soft_isolation()' " << std::endl;
			exit(1); 
		}
		...
		int threshold_Invalid = pages_no_per_block / 10; // 1/4 would be fine
		threshold_Invalid *= 9;
		if ((pbke->Blocks[wl_candidate_block_id].Invalid_page_count)< threshold_Invalid) {
			std::cout << "Don't Isolate. page move for Isolation, Valids: " << pages_no_per_block - pbke->Blocks[wl_candidate_block_id].Invalid_page_count << std::endl;
			return;
		}
		else {
			std::cout << "Isolate. Valids: " << pages_no_per_block - pbke->Blocks[wl_candidate_block_id].Invalid_page_count << std::endl;
		}

		//Run the state machine to protect against race condition
		block_manager->GC_WL_started(wl_candidate_block_id);
		std::cout << "[wl] flag1" << std::endl;
		...
		if (block_manager->Can_execute_gc_wl(wl_candidate_address))//If there are ongoing requests targeting the candidate block, the gc execution should be postponed
		{
			...
			if (block->Current_page_write_index - block->Invalid_page_count > 0)//If there are some valid pages in block, then prepare flash transactions for page movement
			{
				std::cout << "[wl] flag4" << std::endl;
				NVM_Transaction_Flash_RD* wl_read = NULL;
				NVM_Transaction_Flash_WR* wl_write = NULL;
				//have to change here -> only a few pages have to be moved [doodu]
				for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++)
				{
			...
			std::cout << "[wl] flag5" << std::endl;
			tsu->Submit_transaction(wl_erase_tr);
			std::cout << "[wl] flag6" << std::endl;
			tsu->Schedule();
		}
	}
#endif
	...
	{
		//std::cout << "[debug] run_static_wearleveling" << std::endl;
		PlaneBookKeepingType* pbke = block_manager->Get_plane_bookkeeping_entry(plane_address);
		flash_block_ID_type wl_candidate_block_id = block_manager->Get_coldest_block_id(plane_address);
		if (!is_safe_gc_wl_candidate(pbke, wl_candidate_block_id))
			return;
		...
		if (block_manager->Can_execute_gc_wl(wl_candidate_address))//If there are ongoing requests targeting the candidate block, the gc execution should be postponed
		{
			...
			if (block->Current_page_write_index - block->Invalid_page_count > 0)//If there are some valid pages in block, then prepare flash transactions for page movement
			{
				...
				for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++)
				{
					if (block_manager->Is_page_valid(block, pageID))
					{
						std::cout << "[debug] Copying pages: run_static_wearleveling" << std::endl;
						Stats::Total_page_movements_for_gc;
						wl_candidate_address.PageID = pageID;
						if (use_copyback)
						{
