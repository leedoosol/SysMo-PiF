...
namespace SSD_Components
{
	...
	public:
		...
		* GC should go on in non-preemptible mode.*/
		...
		struct cand_blks Check_gc_required(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address, int nextVictim);
		int Check_gc_requiredForce(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address, int nextVictim, int isSubblock);
		int Check_gc_required4align(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address, int nextVictim);
	private:
